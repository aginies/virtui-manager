    ...
    for i, action in enumerate(actions):
        if action["type"] == "move_volume":
            ...
            if not dest_pool_name:
                log(
                    f"[yellow]Skipping volume '{action['volume_name']}' - No destination pool selected.[/]"
                )
                continue
            ...
            try:
                ...
                if old_backing_path:
                    ...
                    if old_backing_path in path_mapping:
                        ...
                        log(
                            f"  - [yellow]Warning:[/] Backing file '{old_backing_path}' not found in migration plan. Overlay might be broken."
                        )
            except Exception as e:
                log(f"  - [yellow]Warning:[/] Could not check backing chain: {e}")

            log(
                f"Copying volume '[b]{action['volume_name']}[/b]' to pool '[b]{dest_pool_name}[/b]' on destination server '[b]{dest_server_name}[/b]'..."
            )
            ...
            try:
                resolved_backing_path = new_backing_path or ""
                result = copy_volume_across_hosts(
                    ...
                    dest_pool_name,
                    action["volume_name"],
                    new_backing_path=resolved_backing_path,
                ...
                if not new_path:
                    ...
                    try:
                        ...
                        log(
                            f"[red]ERROR: Could not determine new path for volume '{result['new_volume_name']}': {e}[/]"
                        )
                        raise
...
def custom_migrate_vm(
        ...
        if e.get_error_code() == 9:  # VIR_ERR_DOMAIN_EXIST
            ...
            if existing_dest_vm.isActive():
                raise libvirt.libvirtError(
                    f"A VM with the name '{domain.name()}' is running on the destination."
                )
    ...
    actions = [{"type": "vm_metadata", "vm_name": domain.name()}]
    root = ET.fromstring(xml_desc)
    ...
    for i, disk in enumerate(disks):
        ...
        while current_path:
            ...
            try:
                ...
                if not source_vol:
                    log(
                        f"Disk/Backing '{current_path}' is not a managed libvirt volume. Proposing manual copy."
                    )
                    actions.append(
                        {
                            "type": "manual_copy",
                ...
                chain.insert(
                    0, (source_vol, source_pool, current_path)
                )  # Prepend to process backing first
        ...
        for source_vol, source_pool, path in chain:
            ...
            is_overlay = (
                path != disk_path
            )  # If it's not the top-level disk, it's a backing file in this chain context
