...
class ConnectionModal(BaseModal[str | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="connection-dialog"):
            ...
            with Horizontal():
                yield Button(
                    ButtonLabels.CONNECT, variant="primary", id="connect-btn", classes="Buttonpage"
                )
                yield Button(
...
class AddServerModal(BaseModal[Tuple[str, str] | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="add-server-dialog"):
            ...
            yield Checkbox(
                StaticText.AUTOCONNECT_AT_STARTUP, id="autoconnect-checkbox", value=False
            )
            with Horizontal():
                yield Button(
                    ButtonLabels.SAVE, variant="primary", id="save-btn", classes="Buttonpage"
                )
                yield Button(
                    ButtonLabels.CANCEL, variant="default", id="cancel-btn", classes="Buttonpage"
                )
...
class EditServerModal(BaseModal[Tuple[str, str, bool] | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="edit-server-dialog"):
            ...
            yield Label(StaticText.EMPTY_LABEL)
            yield Checkbox(
                StaticText.AUTOCONNECT_AT_STARTUP, id="autoconnect-checkbox", value=self.autoconnect
            )
            with Horizontal():
                yield Button(
                    ButtonLabels.SAVE, variant="primary", id="save-btn", classes="Buttonpage"
        ...
        with Vertical(id="server-management-dialog"):  # , classes="info-details"):
            ...
            with Vertical(classes="server-list"):
                ...
                with Horizontal():
                    ...
                    yield Button(
                        ButtonLabels.CUSTOM_URL, id="custom-conn-btn", classes="Buttonpage"
                    )
                ...
                if confirmed:
                    try:
                        ...
                        self.app.show_success_message(
                            SuccessMessages.SERVER_DELETED_TEMPLATE.format(
                        ...
                        self.app.show_error_message(
                            ErrorMessages.ERROR_DELETING_SERVER_TEMPLATE.format(
                                server_name=server_name_to_delete, error=e
                            )
                        )
