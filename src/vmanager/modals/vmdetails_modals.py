...
class VMDetailModal(ModalScreen):
    ...
    def __init__(
        ...
        selected_domains: list[libvirt.virDomain] = None,
    ) -> None:
        super().__init__()
    ...
    def _run_bulk_operation(
        self, targets, operation, success_msg_fmt, error_msg_fmt, ui_update_callback=None
    ):
        ...
        def worker_func():
            ...
            def finalize():
                progress.dismiss()
                if success_count > 0:
                    msg = success_msg_fmt.format(
                        count=success_count, names=", ".join(successful_names)
                    )
                    self.app.show_success_message(msg)
                    if ui_update_callback:
                        ui_update_callback()
    ...
    def on_mount(self) -> None:
        try:
            ...
            self.app.show_error_message(
                ErrorMessages.VM_DETAIL_COULD_NOT_LOAD_NETWORKS.format(error=e)
            )
            self.available_networks = []
    ...
    def _populate_disks_table(self):
        ...
        for disk in disks_info:
            ...
            if status == "disabled":
                disks_table.add_row(path, device_type, bus, "", "", "(disabled)", key=path)
            else:
                disks_table.add_row(
                    path, device_type, bus, cache_mode, discard_mode, "enabled", key=path
                )
    ...
    def _update_graphics_ui(self) -> None:
        ...
        try:
            password_enable_checkbox = self.query_one(
                "#graphics-password-enable-checkbox", Checkbox
            )
        ...
        try:
            ...
            password_input.value = (
                (self.graphics_info["password"] or "")
    ...
    def on_network_change(self, event: Select.Changed) -> None:
        ...
        mac_address = ":".join(
            mac_address_flat[i : i + 2] for i in range(0, len(mac_address_flat), 2)
        )
        ...
        try:
            ...
            self.app.show_success_message(
                SuccessMessages.VM_NETWORK_INTERFACE_CHANGED_TEMPLATE.format(
                    mac_address=mac_address, new_network=new_network
                )
            )
            for i in self.vm_info["networks"]:
                if i["mac"] == mac_address:
            ...
            self.app.show_error_message(
                ErrorMessages.ERROR_UPDATING_NETWORK_TEMPLATE_SHORT.format(error=e)
            )
    ...
    def on_cpu_model_changed(self, event: Select.Changed) -> None:
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_CPU_MODEL_SET_SUCCESS.format(new_cpu_model=new_cpu_model)
    ...
    def on_uefi_file_changed(self, event: Select.Changed) -> None:
        ...
        msg_template = SuccessMessages.VM_UEFI_FILE_SET_SUCCESS.format(
            uefi_file_name=os.path.basename(new_uefi_path) if new_uefi_path else "BIOS"
        )
        if self.is_bulk:
            msg_template += " for {count} VMs ({names})"

        self._run_bulk_operation(
            targets, operation, msg_template, "Errors setting UEFI file", ui_update
        )
    ...
    def on_video_model_changed(self, event: Select.Changed) -> None:
        ...
        def operation(d):
            set_vm_video_model(
                d, new_model if new_model != "default" else None, accel3d=accel3d_enabled
            )
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_VIDEO_MODEL_SET_SUCCESS.format(new_model=new_model)
            + (" for {count} VMs ({names})" if self.is_bulk else ""),
    ...
    def on_video_3d_accel_changed(self, event: Checkbox.Changed) -> None:
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_3D_ACCELERATION_SET_SUCCESS.format(
                state="enabled" if accel3d_enabled else "disabled"
            )
            + (" for {count} VMs ({names})" if self.is_bulk else ""),
    ...
    def on_sound_model_changed(self, event: Select.Changed) -> None:
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_SOUND_MODEL_SET_SUCCESS.format(new_model=new_model)
            + (" for {count} VMs ({names})" if self.is_bulk else ""),
    ...
    def on_secure_boot_checkbox_changed(self, event: Checkbox.Changed) -> None:
        ...
        if (
            not current_uefi_path and event.value
        ):  # Trying to enable secure boot without a UEFI file
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_SECURE_BOOT_SET_SUCCESS.format(
    ...
    def on_shared_memory_changed(self, event: Checkbox.Changed) -> None:
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.VM_SHARED_MEMORY_SET_SUCCESS.format(
                state="enabled" if event.value else "disabled"
            )
            + (" for {count} VMs ({names})" if self.is_bulk else ""),
    ...
    def on_graphics_apply_button_pressed(self, event: Button.Pressed) -> None:
        ...
        logging.info(
            f"Attempting to change graphics from '{original_graphics_type}' to '{new_graphics_type}'."
        )
        ...
        def do_apply_graphics_settings():
            ...
            self._run_bulk_operation(
                targets,
                operation,
                SuccessMessages.GRAPHICS_SETTINGS_APPLIED_SUCCESSFULLY
            ...
            def on_confirm_spice_removal(confirmed: bool):
                if confirmed:
                    ...
                    self._run_bulk_operation(
                        targets,
                        removal_operation,
                        SuccessMessages.SPICE_DEVICES_REMOVED_SUCCESS
    ...
    def on_rng_apply_button_pressed(self, event: Button.Pressed) -> None:
        ...
        self._run_bulk_operation(
            targets,
            operation,
            SuccessMessages.RNG_SETTINGS_APPLIED_SUCCESSFULLY.format(rng_device=rng_device)
            + (" (Applied to {count} VMs ({names}))" if self.is_bulk else ""),
    ...
    def on_tpm_type_changed(self, event: Select.Changed) -> None:
        if self.tpm_info:
            ...
            self.tpm_info = [
                {"type": event.value, "model": "tpm-crb"}
    ...
    def on_tpm_model_changed(self, event: Select.Changed) -> None:
        if self.tpm_info:
            ...
            self.tpm_info = [
                {"model": event.value, "type": "emulated"}
            ]  # Default type if none exists
        self._update_tpm_ui()
        ...
        def ui_update():
            if not self.is_bulk:
                ...
                self._run_bulk_operation(
                    targets,
                    operation,
                    SuccessMessages.TPM_SETTINGS_APPLIED_SUCCESSFULLY
                    + (" (Applied to {count} VMs ({names}))" if self.is_bulk else ""),
    ...
    def on_boot_order_list_highlighted(self, event: ListView.Highlighted) -> None:
        ...
        if (
            event.item
            and boot_list.index is not None
            and boot_list.index < len(boot_list.children) - 1
        ):
    ...
    def on_attach_usb_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if available_list.highlighted_child:
            ...
            try:
                ...
                self._invalidate_cache()
                self.app.show_success_message(
                    SuccessMessages.USB_DEVICE_ATTACHED_TEMPLATE.format(
                ...
                self.app.show_error_message(
                    ErrorMessages.ERROR_ATTACHING_USB_DEVICE_TEMPLATE.format(error=e)
                )
    ...
    def on_detach_usb_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if attached_list.highlighted_child:
            ...
            try:
                ...
                self._invalidate_cache()
                self.app.show_success_message(
                    SuccessMessages.USB_DEVICE_DETACHED_TEMPLATE.format(
                ...
                self.app.show_error_message(
                    ErrorMessages.ERROR_DETACHING_USB_DEVICE_TEMPLATE.format(error=e)
                )
        ...
        for controller in usb_controllers:
            if controller.get("type") == "controller":
                all_controllers.append(
                    {"type": "USB", "model": controller["model"], "index": controller["index"]}
                )
        for controller in scsi_controllers:
            if controller.get("type") == "controller":
                all_controllers.append(
                    {"type": "SCSI", "model": controller["model"], "index": controller["index"]}
                )
    ...
    def on_add_channel_btn_pressed(self, event: Button.Pressed) -> None:
        def add_channel_callback(result):
            if result:
                try:
                    add_vm_channel(self.domain, result["type"], result["target_name"])
                    self._invalidate_cache()
                    self.app.show_success_message(SuccessMessages.CHANNEL_ADDED_SUCCESSFULLY)
                    self._update_channel_table()
                except libvirt.libvirtError as e:
                    self.app.show_error_message(
                        ErrorMessages.ERROR_ADDING_CHANNEL_TEMPLATE.format(error=e)
                    )
            ...
            def on_confirm(confirmed: bool) -> None:
                if confirmed:
                    try:
                        ...
                        self._invalidate_cache()
                        self.app.show_success_message(
                            SuccessMessages.CHANNEL_REMOVED_SUCCESSFULLY_TEMPLATE.format(
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="vm-detail-container"):
            ...
            yield Button(
                ButtonLabels.OTHER_TABS, id="toggle-detail-button", classes="toggle-detail-button"
            )
            with TabbedContent(id="detail-vm"):
                with TabPane("CPU", id="detail-cpu-tab"):
                    with Vertical(classes="info-details"):
                        yield Label(
                            f"CPU: {self.vm_info.get('cpu', 'N/A')}", id="cpu-label", classes="tabd"
                        )
                        ...
                        yield Label(
                            f"CPU Pinning: {vcpupin_count} rules",
                            id="cputune-label",
                            classes="tabd",
                        )
                        yield Button(
                            ButtonLabels.EDIT_CPU_TUNE,
                            id="edit-cputune",
                            classes="edit-detail-btn",
                            disabled=not self.is_vm_stopped,
                        )
                        ...
                        yield Button(
                            ButtonLabels.EDIT_NUMA_TUNE,
                            id="edit-numatune",
                            classes="edit-detail-btn",
                            disabled=numa_btn_disabled,
                        )
                ...
                with TabPane(
                    ...
                    with Vertical(classes="info-details"):
                        yield Label(
                            f"Memory: {self.vm_info.get('memory', 'N/A')} MB",
                            id="memory-label",
                            classes="tabd",
                        )
                        ...
                        yield Checkbox(
                            "Shared Memory",
                            value=self.vm_info.get("shared_memory", False),
                            id="shared-memory-checkbox",
                            classes="shared-memory",
                            disabled=not self.is_vm_stopped,
                        )
                if not self.is_bulk:
                    with TabPane("Firmware", id="detail-firmware-tab"):
                        with Vertical(classes="info-details"):
                            ...
                            if firmware_type == "UEFI":
                                ...
                                yield Checkbox(
                                    "AMD-SEV", id="sev-checkbox", disabled=not self.is_vm_stopped
                                )
                                yield Checkbox(
                                    "AMD-SEV-ES",
                                    id="sev-es-checkbox",
                                ...
                                yield Static(classes="button-separator")
                                yield Button(
                                    ButtonLabels.SWITCH_TO_BIOS,
                                    id="switch-to-bios",
                                    disabled=not self.is_vm_stopped,
                                )
                            else:
                                yield Button(
                                    ButtonLabels.SWITCH_TO_UEFI,
                                    id="switch-to-uefi",
                                    disabled=not self.is_vm_stopped,
                                )
                            ...
                            if "machine_type" in self.vm_info:
                                yield Static(classes="button-separator")
                                yield Label(
                                    f"Machine Type: {self.vm_info['machine_type']}",
                                    id="machine-type-label",
                                    classes="tabd",
                                )
                                yield Button(
                                    ButtonLabels.EDIT,
                        ...
                        with Vertical():
                            ...
                            with Horizontal(classes="boot-manager"):
                                with Vertical(classes="boot-main-container"):
                                    ...
                                    yield ListView(
                                        id="boot-order-list", classes="boot-list-container"
                                    )
                                with Vertical(classes="boot-buttons-container"):
                                    yield Label(StaticText.EMPTY_LABEL)
                                    yield Button(
                                        "<", id="boot-add", disabled=not self.is_vm_stopped
                                    )
                                    yield Button(
                                        ">", id="boot-remove", disabled=not self.is_vm_stopped
                                    )
                                    yield Button(
                                        ButtonLabels.BOOT_UP,
                                        id="boot-up",
                                        disabled=not self.is_vm_stopped,
                                    )
                                    yield Button(
                                        ButtonLabels.BOOT_DOWN,
                                        id="boot-down",
                                        disabled=not self.is_vm_stopped,
                                    )
                                with Vertical(classes="boot-main-container"):
                    ...
                    with TabPane("Disks", id="detail-disk-tab"):
                        ...
                        remove_button = Button(
                            ButtonLabels.REMOVE_DISK,
                            id="detail_remove_disk",
                            classes="detail-disks",
                        )
                        disable_button = Button(
                            ButtonLabels.DISABLE_DISK,
                            id="detail_disable_disk",
                            classes="detail-disks",
                        )
                        enable_button = Button(
                            ButtonLabels.ENABLE_DISK,
                            id="detail_enable_disk",
                            classes="detail-disks",
                        )
                        ...
                        with Vertical(classes="button-details"):
                            with Horizontal():
                                ...
                                yield Button(
                                    ...
                                    id="detail_attach_disk",
                                    classes="detail-disks",
                                )
                                yield Button(
                                    ButtonLabels.EDIT_DISK,
                                    id="detail_edit_disk",
                                    classes="detail-disks",
                                    disabled=True,
                                )
                    ...
                    with TabPane("Networks", id="networks"):
                        ...
                        with Vertical(classes="button-details"):
                            with Horizontal():
                                ...
                                yield Button(
                                    ButtonLabels.ADD_INTERFACE,
                                    id="add-network-interface-button",
                                    classes="detail-disks",
                                    variant="primary",
                                )
                                yield Button(
                                    ButtonLabels.REMOVE_INTERFACE,
                                    id="remove-network-interface-button",
                                    classes="detail-disks",
                                    variant="error",
                                    disabled=True,
                                )
                    ...
                    if self.vm_info.get("devices"):
                        with TabPane("VirtIO-FS", id="detail-virtiofs-tab"):
                            ...
                            lbl = Label(
                                WarningMessages.VIRTIOFS_SHARED_MEM_WARNING,
                                id="virtiofs-shared-mem-warning",
                                classes="tabd-warning",
                            )
                            ...
                            with Vertical(classes="button-details"):
                                with Horizontal():
                                    ...
                                    yield Button(
                                        ButtonLabels.DELETE,
                                        variant="error",
                                        id="delete-virtiofs-btn",
                                        disabled=True,
                                        classes="detail-disks",
                                    )
                                    yield Button(
                                        ButtonLabels.VIRTIOFS_HELP,
                                        id="detail_virtiofs_help",
                                        classes="detail-disks",
                                    )
                ...
                with TabPane("Video", id="detail-video-tab"):
                    with Vertical(classes="info-details"):
                        ...
                        try:
                            ...
                            arch = (
                                os_type_elem.get("arch") if os_type_elem is not None else "x86_64"
                            )
                            machine = (
                                os_type_elem.get("machine") if os_type_elem is not None else None
                            )
                            ...
                            if machine and emulatorbin:
                                caps_xml = get_domain_capabilities_xml(
                                    self.conn, emulatorbin, arch, machine
                                )
                                if caps_xml:
                ...
                with TabPane("Graphics", id="detail-graphics-tab"):
                    with ScrollableContainer(classes="info-details"):
                        ...
                        with RadioSet(
                            id="graphics-address-radioset",
                            disabled=not self.is_vm_stopped
                            or self.graphics_info["listen_type"] != "address",
                        ):
                            yield RadioButton(
                        ...
                        yield Input(
                            ...
                            disabled=not self.is_vm_stopped
                            or not self.graphics_info["password_enabled"],
                        )
                    yield Button(
                        ButtonLabels.APPLY_GRAPHICS_SETTINGS,
                        id="graphics-apply-btn",
                        variant="primary",
                        disabled=not self.is_vm_stopped,
                    )
                with TabPane("TPM", id="detail-tpm-tab"):
                    ...
                    tpm_device_path = (
                        self.tpm_info[0].get("device_path", "") if self.tpm_info else ""
                    )
                    tpm_backend_type = (
                        self.tpm_info[0].get("backend_type", "") if self.tpm_info else ""
                    )
                    tpm_backend_path = (
                        self.tpm_info[0].get("backend_path", "") if self.tpm_info else ""
                    )
                    ...
                    yield Button(
                        ButtonLabels.APPLY_TPM_SETTINGS,
                        id="apply-tpm-btn",
                        variant="primary",
                        disabled=not self.is_vm_stopped,
                    )
            ...
            with TabbedContent(id="detail2-vm"):
                ...
                with TabPane("Serial", id="detail-serial-tab"):
                    ...
                    with Vertical(classes="button-details"):
                        with Horizontal():
                            yield Button(
                                ButtonLabels.ADD_PTY_CONSOLE,
                                id="add-serial-btn",
                                variant="primary",
                                disabled=not self.is_vm_stopped,
                            )
                            yield Button(
                                ButtonLabels.REMOVE_CONSOLE,
                                id="remove-serial-btn",
                                variant="error",
                                disabled=True,
                            )
                ...
                if not self.is_bulk:
                    with TabPane("Input", id="detail-input-tab"):
                        ...
                        with Vertical(classes="button-details"):
                            with Horizontal():
                                yield Button(
                                    ButtonLabels.ADD_INPUT,
                                    id="add-input-btn",
                                    variant="primary",
                                    disabled=not self.is_vm_stopped,
                                )
                                yield Button(
                                    ButtonLabels.REMOVE_INPUT,
                                    id="remove-input-btn",
                                    variant="error",
                                    disabled=True,
                                )
                ...
                with TabPane("Controller", id="detail-controler-tab"):
                    ...
                    with Vertical(classes="button-details"):
                        with Horizontal():
                            yield Button(
                                ButtonLabels.ADD_USB2,
                                id="add-usb2-controller-btn",
                                variant="primary",
                                disabled=not self.is_vm_stopped,
                            )
                            yield Button(
                                ButtonLabels.ADD_USB3,
                                id="add-usb3-controller-btn",
                                variant="primary",
                                disabled=not self.is_vm_stopped,
                            )
                            yield Button(
                                ButtonLabels.ADD_SCSI,
                                id="add-scsi-controller-btn",
                                variant="primary",
                                disabled=not self.is_vm_stopped,
                            )
                            yield Button(
                                ButtonLabels.REMOVE,
                                id="remove-controller-btn",
                                variant="error",
                                disabled=True,
                            )
                if not self.is_bulk:
                ...
                with TabPane("Channel", id="detail-channel-tab"):
                    ...
                    with Vertical(classes="button-details"):
                        with Horizontal():
                            yield Button(
                                ButtonLabels.ADD_CHANNEL,
                                id="add-channel-btn",
                                variant="primary",
                                disabled=not self.is_vm_stopped,
                            )
                            yield Button(
                                ButtonLabels.REMOVE_CHANNEL,
    ...
    def _update_tpm_ui(self) -> None:
        ...
        try:
            ...
            tpm_model_select.value = (
                self.tpm_info[0].get("model", "none") if self.tpm_info else "none"
            )
        ...
        try:
            ...
            tpm_type_select.value = (
                self.tpm_info[0].get("type", "emulated") if self.tpm_info else "emulated"
            )
        ...
        try:
            ...
            device_path_input.value = (
                self.tpm_info[0].get("device_path", "") if self.tpm_info else ""
            )
            device_path_input.disabled = not self.is_vm_stopped or (
        ...
        try:
            ...
            backend_type_input.value = (
                self.tpm_info[0].get("backend_type", "") if self.tpm_info else ""
            )
            backend_type_input.disabled = not self.is_vm_stopped or (
                self.tpm_info[0].get("type") != "passthrough" if self.tpm_info else True
            )
        except Exception:
            pass
        ...
        try:
            ...
            backend_path_input.value = (
                self.tpm_info[0].get("backend_path", "") if self.tpm_info else ""
            )
            backend_path_input.disabled = not self.is_vm_stopped or (
                self.tpm_info[0].get("type") != "passthrough" if self.tpm_info else True
            )
        except Exception:
            pass
        ...
        if event.button.id == "toggle-detail-button":
            ...
            def add_virtiofs_callback(result):
                if result:
                    try:
                        ...
                        self._invalidate_cache()
                        self.app.show_success_message(
                            f"VirtIO-FS mount '{result['target_path']}' added successfully."
                        )
            ...
            if self.selected_network_interface:
                interface_to_edit = next(
                    (
                        net
                        for net in self.vm_info["networks"]
                        if net["mac"] == self.selected_network_interface
                    ),
                    None,
                )
                if interface_to_edit:
                    ...
                    def edit_interface_callback(result):
                        if result:
                            ...
                            if (
                                new_network_name == original_network_name
                                and new_model == original_model
                            ):
                                self.app.show_success_message(
                                    "No changes detected for network interface."
                                )
                                return

                            message = (
                                f"Are you sure you want to modify network interface\n'{original_mac}'\n"
                            ...
                            def on_confirm_edit(confirmed: bool) -> None:
                                if confirmed:
                                    try:
                                        ...
                                        self.app.show_error_message(
                                            f"Error modifying network interface: {e}"
                                        )
                                    except Exception as e:
                                        self.app.show_error_message(
            ...
            def add_interface_callback(result):
                if result:
                    try:
                        add_network_interface(self.domain, result["network"], result["model"])
                        self._invalidate_cache()
            ...
            self.app.push_screen(
                AddEditNetworkInterfaceModal(
                    is_edit=False, networks=self.available_networks, network_models=network_models
                ),
                ...
                def on_confirm(confirmed: bool) -> None:
                    if confirmed:
                        try:
                            ...
                            self._invalidate_cache()
                            self.app.show_success_message(
                                f"Network interface '{self.selected_network_interface}' removed successfully."
                            )
                            self._update_networks_table()
                        except (libvirt.libvirtError, ValueError) as e:
            ...
            def on_confirm_bios_switch(confirmed: bool):
                if confirmed:
                    try:
                        ...
                        self._invalidate_cache()
                        self.app.show_success_message(
                            "Switched to BIOS successfully. Please reopen the dialog to see changes."
                        )
                        self.dismiss()
                    except (libvirt.libvirtError, ValueError) as e:
            ...
            self.app.push_screen(
                ConfirmationDialog(
                    "Are you sure you want to switch to BIOS? This may affect bootability."
                ),
            ...
            def on_uefi_file_selected(uefi_path: str | None):
                if uefi_path:
                    try:
                        ...
                        self._invalidate_cache()
                        self.app.show_success_message(
                            "Switched to UEFI successfully. Please reopen the dialog to see changes."
                        )
                        self.dismiss()
                    except (libvirt.libvirtError, ValueError) as e:
                        ...
                        if confirmed:
                            ...
                            def migrate_worker():
                                try:
                                    ...
                                    migrate_vm_machine_type(
                                        self.domain, new_machine_type, log_callback=log_callback
                                    )
                                    ...
                                    self.app.call_from_thread(
                                        self.app.show_error_message,
                                        f"Libvirt error during machine type migration: {e}",
                                    )
                                except Exception as e:
                                    self.app.call_from_thread(
                                        self.app.show_error_message,
                                        f"Unexpected error during machine type migration: {e}",
                                    )

                            worker_id = self.vm_service._get_internal_id(self.domain)
                            self.app.run_worker(
                                migrate_worker,
                                name=f"migrate_machine_type_{worker_id}",
                                thread=True,
                            )
                        else:
            ...
            self.app.push_screen(
                SelectMachineTypeModal(supported_machine_types, current_machine_type),
            ...
            def select_pool_callback(pool_name: str | None) -> None:
                ...
                self.app.push_screen(
                    SelectDiskModal(
                        all_volume_paths, f"Select a disk to attach from pool '{pool_name}'"
                    ),
            ...
            def edit_disk_callback(result):
                if result:
                    ...
                    if (
                        ...
                        and new_bus == selected_disk.get("bus")
                        and new_device_type == selected_disk.get("device_type")
                    ):
                    ...
                    try:
                        ...
                        set_disk_properties(
                            self.domain, selected_disk.get("path"), properties=disk_properties
                        )
                        self._invalidate_cache()
                        self.app.show_success_message(
    ...
    def on_edit_cputune_pressed(self, event: Button.Pressed) -> None:
        ...
        self.app.push_screen(
            EditCpuTuneModal(self.cputune_info.get("vcpupin"), max_vcpus=current_vcpu_count),
            callback,
        )
