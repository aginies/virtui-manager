...
class SelectPoolModal(BaseModal[str | None]):
    ...
    def __init__(self, pools: list[str], prompt: str) -> None:
        """Initialize the modal with pool list and prompt."""
        super().__init__()
        self.pools = pools
        self.prompt = prompt
...
class SelectDiskModal(BaseModal[str | None]):
    ...
    def __init__(self, disks: list[str], prompt: str) -> None:
        """Initialize the modal with disk list and prompt."""
        super().__init__()
        self.disks = disks
        self.prompt = prompt
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="select-disk-dialog"):
            ...
            with ScrollableContainer():
                yield ListView(
                    *[
                        ValueListItem(Label(os.path.basename(disk)), value=disk)
...
class AddDiskModal(BaseModal[dict | None]):
    ...
    def compose(self) -> ComposeResult:
        with ScrollableContainer(id="add-disk-dialog"):
            ...
            yield Input(
                placeholder=StaticText.VOLUME_NAME_PLACEHOLDER,
                id="volume-name-input",
                disabled=True,
            )
            yield Input(
                placeholder=StaticText.DISK_SIZE_PLACEHOLDER, id="disk-size-input", disabled=True
            )
            yield Select(
                [("qcow2", "qcow2"), ("raw", "raw")],
                id="disk-format-select",
                disabled=True,
                value="qcow2",
            )
            ...
            with Horizontal():
                yield Button(
                    ButtonLabels.ADD, variant="primary", id="add-btn", classes="Buttonpage"
                )
                yield Button(
                    ButtonLabels.CANCEL, variant="default", id="cancel-btn", classes="Buttonpage"
                )
    ...
    def on_create_disk_checkbox_changed(self, event: Checkbox.Changed) -> None:
        ...
        if is_creating:
            ...
            if cdrom_checkbox.value:
                cdrom_checkbox.value = False
                self.on_cdrom_checkbox_changed(
                    Checkbox.Changed(cdrom_checkbox, value=False)
                )  # Trigger its handler
    ...
    def on_cdrom_checkbox_changed(self, event: Checkbox.Changed) -> None:
        ...
        if event.value:
            ...
            bus_select.set_options(
                [("sata", "sata"), ("ide", "ide"), ("scsi", "scsi"), ("usb", "usb")]
            )
...
class AddPoolModal(BaseModal[bool | None]):
    ...
    def on_pool_type_select_changed(self, event: Select.Changed) -> None:
        """Handle pool type selection changes."""
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id in ("browse-dir-btn", "browse-netfs-btn"):
            ...
            self.app.push_screen(
                DirectorySelectionModal(path=input_to_update.value), on_directory_selected
            )
            return
            ...
            if was_modified:
                self.app.show_success_message(
                    SuccessMessages.INPUT_SANITIZED.format(
                        original_input=pool_name_raw, sanitized_input=pool_name
                    )
                )
            ...
            def do_create_pool():
                try:
                    ...
                    self.app.call_from_thread(
                        self.app.show_success_message,
                        SuccessMessages.STORAGE_POOL_CREATED_TEMPLATE.format(
...
class CreateVolumeModal(BaseModal[dict | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="create-volume-dialog"):
            ...
            yield Input(
                placeholder=StaticText.DISK_SIZE_PLACEHOLDER, id="vol-size-input", type="integer"
            )
            yield Select(
                [("qcow2", "qcow2"), ("raw", "raw")], id="vol-format-select", value="qcow2"
            )
            with Horizontal():
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "create-btn":
            ...
            if was_modified:
                self.app.show_success_message(
                    SuccessMessages.INPUT_SANITIZED.format(
                        original_input=name_raw, sanitized_input=name
                    )
                )
...
class EditDiskModal(BaseModal[dict | None]):
    """Modal screen for editing an existing disk."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="edit-disk-dialog"):
            yield Label(
                StaticText.EDIT_DISK_TITLE.format(path=self.disk_info["path"]), id="edit-disk-title"
            )

            yield Label(StaticText.DEVICE_TYPE)
            yield Select(
                device_options,
                value=self.disk_info.get("device_type") or "disk",
                id="edit-device-type",
                disabled=not self.is_stopped,
            )

            yield Label(StaticText.BUS_TYPE)
            yield Select(
                bus_options,
                value=self.disk_info.get("bus"),
                id="edit-bus-type",
                disabled=not self.is_stopped,
            )
            ...
            yield Select(
                cache_options,
                value=self.disk_info.get("cache_mode") or "none",
                id="edit-cache-mode",
                disabled=not self.is_stopped,
            )
            ...
            yield Select(
                discard_options,
                value=self.disk_info.get("discard_mode") or "unmap",
                id="edit-discard-mode",
                disabled=not self.is_stopped,
            )
...
class MoveVolumeModal(BaseModal[dict]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="move-volume-dialog"):
            yield Label(
                StaticText.MOVE_VOLUME_TITLE.format(volume_name=self.volume_name),
                id="move-volume-title",
            )
            yield Static(
                StaticText.FROM_POOL.format(source_pool_name=self.source_pool_name),
                classes="label-like",
            )
            ...
            dest_pools = [
                (p["name"], p["name"])
                for p in pools
                if p["name"] != self.source_pool_name and p["status"] == "active"
            ]
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "cancel-btn":
            ...
            if was_modified:
                self.app.show_success_message(
                    SuccessMessages.INPUT_SANITIZED.format(
                        original_input=new_name_input_raw, sanitized_input=new_name
                    )
                )

            if dest_pool_select.value and new_name:
                self.dismiss({"dest_pool": dest_pool_select.value, "new_name": new_name})
            else:
...
class AttachVolumeModal(BaseModal[dict | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="attach-volume-dialog"):
            ...
            yield Input(
                placeholder=StaticText.EXISTING_VOLUME_NAME_PLACEHOLDER,
                id="vol-name-input",
                disabled=True,
            )
            with Horizontal():
    ...
    def on_vol_path_input_changed(self, event: Input.Changed) -> None:
        """Handle volume path input changes."""
        if event.value:
            ...
            try:
                ...
                if was_modified:
                    self.app.show_success_message(
                        SuccessMessages.INPUT_SANITIZED.format(
                            original_input=os.path.basename(event.value),
                            sanitized_input=sanitized_name,
                        )
                    )
