...
class HostDashboardModal(BaseModal[None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="host-dashboard-dialog"):
            yield Label(
                f"{StaticText.HOST_RESOURCE_DASHBOARD} - {self.server_name}", id="dialog-title"
            )
            ...
            with Vertical(classes="info-section-dashboard"):
                ...
                with Grid(classes="host-info-grid"):
                    ...
                    yield Label("CPUs:")
                    yield Label(
                        f"{self.host_resources.get('total_cpus', 0)} ({self.host_resources.get('mhz', 0)} MHz)"
                    )
                    yield Label(StaticText.TOPOLOGY)
                    yield Label(
                        f"{self.host_resources.get('nodes', 1)} nodes, {self.host_resources.get('sockets', 1)} sockets, {self.host_resources.get('cores', 1)} cores, {self.host_resources.get('threads', 1)} threads"
                    )
            ...
            with Vertical(classes="info-section-dashboard"):
                ...
                with Horizontal(classes="usage-row"):
                    yield Label(
                        f"{used_mem/1024:.1f} GB / {total_mem/1024:.1f} GB ({mem_pct:.1f}%)"
                    )
                    yield ProgressBar(
                        total=100, show_percentage=False, show_eta=False, id="mem-bar"
                    )
            ...
            with Vertical(classes="info-section-dashboard"):
                ...
                with TabbedContent():
                    with TabPane(TabTitles.ACTIVE_ALLOCATION, id="tab-active"):
                        ...
                        yield Label(
                            f"{StaticText.ALLOCATED_VCPUS} (Active):", id="cpu-alloc-label-active"
                        )
                        yield ProgressBar(
                            total=100,
                            show_percentage=False,
                            show_eta=False,
                            id="cpu-alloc-bar-active",
                        )
                        yield Rule()
                        # Active Memory Allocation
                        yield Label(
                            f"{StaticText.ALLOCATED_MEMORY} (Active):", id="mem-alloc-label-active"
                        )
                        yield ProgressBar(
                            total=100,
                            show_percentage=False,
                            show_eta=False,
                            id="mem-alloc-bar-active",
                        )
                    with TabPane(TabTitles.TOTAL_ALLOCATION, id="tab-total"):
                        # Total CPU Allocation
                        yield Label(
                            f"{StaticText.ALLOCATED_VCPUS} (Total):", id="cpu-alloc-label-total"
                        )
                        yield ProgressBar(
                            total=100,
                            show_percentage=False,
                            show_eta=False,
                            id="cpu-alloc-bar-total",
                        )
                        yield Rule()
                        # Total Memory Allocation
                        yield Label(
                            f"{StaticText.ALLOCATED_MEMORY} (Total):", id="mem-alloc-label-total"
                        )
                        yield ProgressBar(
                            total=100,
                            show_percentage=False,
                            show_eta=False,
                            id="mem-alloc-bar-total",
                        )
    ...
    def update_dashboard(self) -> None:
        ...
        self.query_one("#cpu-alloc-label-total", Label).update(
            f"{StaticText.ALLOCATED_VCPUS} (Total): {alloc_cpus_total} / {total_host_cpus} ({cpu_alloc_pct_total:.1f}%)"
        )
        ...
        self.query_one("#mem-alloc-label-total", Label).update(
            f"{StaticText.ALLOCATED_MEMORY} (Total): {alloc_mem_total/1024:.1f} GB / {total_mem/1024:.1f} GB ({mem_alloc_pct_total:.1f}%)"
        )
        ...
        self.query_one("#cpu-alloc-label-active", Label).update(
            f"{StaticText.ALLOCATED_VCPUS} (Active): {alloc_cpus_active} / {total_host_cpus} ({cpu_alloc_pct_active:.1f}%)"
        )
        ...
        self.query_one("#mem-alloc-label-active", Label).update(
            f"{StaticText.ALLOCATED_MEMORY} (Active): {alloc_mem_active/1024:.1f} GB / {total_mem/1024:.1f} GB ({mem_alloc_pct_active:.1f}%)"
        )
