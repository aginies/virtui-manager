...
class VirtuiWrapper(Gtk.Window):
    def __init__(self):
        ...
        if check_tmux():
            cmd_wrapper = ["tmux", "new-session", "-A", "-s", "virtui-manager", " ".join(cmd_wrapper)]
        self.create_tab("Virtui Manager", cmd_wrapper, is_main_app=True)
    ...
    def create_tab(self, title, command, is_main_app=False):
        ...
        # Pass is_main_app as user_data to the signal handler
        terminal.connect("child-exited", self.on_child_exited, is_main_app)
        ...
        self.notebook.append_page(scrolled_window, Gtk.Label(label=title))
    ...
    def on_child_exited(self, terminal, status, is_main_app):
