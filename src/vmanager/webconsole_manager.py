...
"""
    ...
    def start_console(self, vm, conn):
        ...
        if self.is_running(uuid):
            ...
            def on_dialog_dismiss(result):
                if result == "stop":
                    self.app.worker_manager.run(stopper_worker, name=f"stop_console_{uuid}")
        ...
        try:
            ...
            if graphics_info.get("type") != "vnc":
                self.app.call_from_thread(
                    self.app.show_error_message, ErrorMessages.WEBCONSOLE_VNC_ONLY
                )
                return
            ...
            if not vnc_port or vnc_port == "-1":
                self.app.call_from_thread(
                    self.app.show_error_message, ErrorMessages.COULD_NOT_DETERMINE_VNC_PORT
                )
                return
            ...
            if is_remote_ssh and self.config.get("REMOTE_WEBCONSOLE", False):
                ...
                if vnc_target_host and vnc_target_port:
                    self._launch_websockify(
                        uuid, vm_name, vnc_target_host, vnc_target_port, ssh_info
                    )

        except (libvirt.libvirtError, FileNotFoundError, Exception) as e:
            self.app.call_from_thread(
                self.app.show_error_message,
                ErrorMessages.FAILED_TO_START_WEBCONSOLE_TEMPLATE.format(error=e),
            )
    ...
    def stop_console(self, uuid: str, vm_name: str):
        ...
        if remote_pid and remote_host:
            try:
                ...
                logging.warning(
                    f"Failed to kill remote websockify {remote_pid} on {remote_host}: {e}"
                )
    ...
    def _launch_remote_websockify(
        self, uuid: str, vm_name: str, conn, vnc_port: int, graphics_info: dict
    ):
        """
        ...
        try:
            ...
            if stdout_lines:
                ...
                if "user_cert" in cert_status:
                    url_scheme = "https"
                    self.app.call_from_thread(
                        self.app.show_success_message, SuccessMessages.REMOTE_USER_CERT_FOUND
                    )
                    remote_websockify_cmd_list.extend(
                        ["--cert", remote_cert_file, "--key", remote_key_file]
                    )
                elif "system_cert" in cert_status:
                    url_scheme = "https"
                    self.app.call_from_thread(
                        self.app.show_success_message, SuccessMessages.REMOTE_SYSTEM_CERT_FOUND
                    )
                    remote_websockify_cmd_list.extend(
                        ["--cert", system_cert_file, "--key", system_key_file]
                    )
                else:
                    # Attempt to generate keys remotely
                    self.app.call_from_thread(
                        self.app.show_success_message,
                        SuccessMessages.NO_REMOTE_CERT_FOUND_GENERATING,
                    )
                    gen_msgs = generate_webconsole_keys_if_needed(
            ...
            logging.warning(
                f"Could not check for remote certs/novnc: {e}. Proceeding without SSL options and default novnc path."
            )
            self.app.call_from_thread(
        ...
        try:
            if proc.stdout:
                ...
                logging.warning("Failed to get remote PID: stdout is None")
        except Exception as e:
        ...
        if remote_pid is None:
            self.app.call_from_thread(
                self.app.show_error_message,
                ErrorMessages.FAILED_TO_START_REMOTE_WEBSOCKIFY_TEMPLATE.format(vm_name=vm_name),
            )
            try:
                proc.terminate()
            except Exception:
        ...
        if not local_tunnel_port:
            ...
            try:
                ...
                pass
            self.app.call_from_thread(
                self.app.show_error_message, ErrorMessages.NO_FREE_LOCAL_PORT_FOR_SSH_TUNNEL
            )
            return
        ...
        try:
            ...
            self.app.call_from_thread(
                self.app.show_error_message,
                ErrorMessages.FAILED_TO_CREATE_SSH_TUNNEL_TEMPLATE.format(error=e),
            )
            return
    ...
    def _setup_ssh_tunnel(
        ...
        self.app.call_from_thread(
            self.app.show_success_message, SuccessMessages.REMOTE_CONNECTION_SSH_TUNNEL_SETUP
        )
        parsed_uri = urlparse(conn.getURI())
        user = parsed_uri.username
        host = parsed_uri.hostname
        ...
        try:
            ...
            self.app.call_from_thread(
                self.app.show_error_message, ErrorMessages.FAILED_TO_CREATE_SSH_TUNNEL_GENERIC
            )
    ...
    def _get_local_novnc_path(self) -> str:
        ...
        config_path = self.config.get("novnc_path")
    ...
    def _launch_websockify(self, uuid: str, vm_name: str, host: str, port: int, ssh_info: dict):
        ...
        with open(log_file_path, "a") as log_file_handle:
            if cert_file.exists() and key_file.exists():
                ...
                url_scheme = "https"
                self.app.call_from_thread(
                    self.app.show_success_message, SuccessMessages.LOCAL_CERT_FOUND
                )
    ...
    def _stop_ssh_tunnel(self, vm_name: str, ssh_info: dict):
        ...
        try:
            ...
            logging.warning(
                f"Could not stop SSH tunnel cleanly for VM {vm_name}: {e.stderr.decode() if e.stderr else e}"
            )
        finally:
